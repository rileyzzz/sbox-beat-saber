...
namespace NVorbis.Ogg
{
    internal class Packet : DataPacket
    {
        ...
        int _dataCount;
		Sandbox.Memory<byte> _data;
        int _dataIndex;                                         // 4
        int _dataOfs;                                           // 4

        internal Packet(IReadOnlyList<int> dataParts, IPacketReader packetReader, Sandbox.Memory<byte> initialData)
        {
        ...
        protected override int ReadNextByte()
        {
            ...
            if (++_dataOfs == _data.Length)
            {
                ...
                {
                    _data = Sandbox.Memory<byte>.Empty;
                }
            }

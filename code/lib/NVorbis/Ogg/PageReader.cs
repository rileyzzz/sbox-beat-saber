...
namespace NVorbis.Ogg
{
    class PageReader : PageReaderBase, IPageData
    {
        ...
        private static Sandbox.Memory<byte>[] ReadPackets(int packetCount, Span<byte> segments, Sandbox.Memory<byte> dataBuffer)
        {
            var list = new Sandbox.Memory<byte>[packetCount];
            var listIdx = 0;
            var dataIdx = 0;
            var size = 0;
        ...
        {
			//Log.Warning("Attempting to check lock");
			return _lockEntered;
        ...
        protected override bool AddPage(int streamSerial, byte[] pageBuf, bool isResync)
        {
            ...
            _packets = ReadPackets(PacketCount, new Span<byte>(pageBuf, 27, pageBuf[26]), new Sandbox.Memory<byte>(pageBuf, 27 + pageBuf[26], pageBuf.Length - 27 - pageBuf[26]));
        ...
        public override bool ReadPageAt(long offset)
        {
            ...
            if (!CheckLock())
			{
				Log.Error( "Must be locked prior to reading!" );
				return false;
			}
        ...
        public Sandbox.Memory<byte>[] GetPackets()
        {

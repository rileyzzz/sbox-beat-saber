...
namespace NVorbis.Ogg
{
    class StreamPageReader : IStreamPageReader
    {
        ...
        private Sandbox.Memory<byte>[] _cachedPagePackets;
            ...
            if (!HasAllPages)
            {
                ...
                if (_reader.GranulePosition != -1)
                {
                    ...
                    else if (_maxGranulePos > _reader.GranulePosition)
                    {
                        ...
                        Log.Error("Granule Position regressed?!");
						return;
                    }
                ...
                else if (_firstDataPageIndex.HasValue && (!_reader.IsContinued || _reader.PacketCount != 1))
                {
					...
					Log.Error( "Granule Position was -1 but page does not have exactly 1 continued packet." );
					return;
                }
        ...
        public Sandbox.Memory<byte>[] GetPagePackets(int pageIndex)
        {

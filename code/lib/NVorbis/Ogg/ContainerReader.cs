...
namespace NVorbis.Ogg
{
    ...
    public sealed class ContainerReader : Contracts.IContainerReader
    {
		...
		// TODO: goofballs over at facepunch still haven't whitelisted WeakReference.
		// changing this probably prevents some shit from getting released. Too bad!
		// https://github.com/Facepunch/sbox-issues/issues/1322
		//private List<WeakReference<Contracts.IPacketProvider>> _packetProviders;
		private List<Contracts.IPacketProvider> _packetProviders;
        ...
        public IReadOnlyList<Contracts.IPacketProvider> GetStreams()
        {
            ...
            for (var i = 0; i < _packetProviders.Count; i++)
            {
                ...
                if (_packetProviders[i] != null)
                {
                    //list.Add(pp);
                    list.Add( _packetProviders[i] );
                }
                else
                {
                    list.RemoveAt(i);
        ...
        public ContainerReader(Stream stream, bool closeOnDispose)
        {
            ...
            _packetProviders = new List<Contracts.IPacketProvider>();
        ...
        private bool ProcessNewStream(Contracts.IPacketProvider packetProvider)
        {
            ...
            try
            {
                if (NewStreamCallback?.Invoke(packetProvider) ?? true)
                {
                    ...
                    _packetProviders.Add(packetProvider);
                    _foundStream = true;
                    return true;
                }
                return false;

	...
	public sealed class StreamDecoder : IStreamDecoder
	{
		...
		internal StreamDecoder( Contracts.IPacketProvider packetProvider, IFactory factory )
		{
			if ( packetProvider == null )
				Log.Error( "Null argument " + nameof( packetProvider ) );

			if ( factory == null )
				Log.Error( "Null argument " + nameof( factory ) );
			...
			if ( !ProcessHeaderPackets( packet ) )
			{
				...
				Log.Error( GetInvalidStreamException( packet ) );
			}
		}

		private static string GetInvalidStreamException( IPacket packet )
		{
			try
			{
		...
		static private string ReadString( IPacket packet )
		{
			...
			if ( cnt < len )
			{
				Log.Error( "Could not read full string!" );
			}
		...
		private bool LoadBooks( IPacket packet )
		{
			...
			if ( !packet.ReadBit() ) Log.Error( "Book packet did not end on correct bit!" );
		...
		{
			if ( buffer == null )
				Log.Error( "Null argument " + nameof( buffer ) );

			if ( offset < 0 || offset + count > buffer.Length )
				Log.Error( "Argument out of range " + nameof( offset ) );

			if ( count % _channels != 0 ) Log.Error( "Argument out of range " + nameof( count ) + " must be a multiple of Channels!" );
			if ( _packetProvider == null ) Log.Error( "ObjectDisposedException " + nameof( StreamDecoder ) );
		...
		{
			if ( _packetProvider == null ) Log.Error( "ObjectDisposedException " + nameof( StreamDecoder ) );
			if ( !_packetProvider.CanSeek ) Log.Error( "Seek is not supported by the Contracts.IPacketProvider instance." );
			...
			switch ( seekOrigin )
			{
				...
				default:
					{
						Log.Error( "Argument out of range " + nameof( seekOrigin ) );
						return;
					}
			}

			if ( samplePosition < 0 ) Log.Error( "Argument out of range " + nameof( samplePosition ) );
			...
			if ( !ReadNextPacket( 0, out _ ) )
			{
				...
				{
					Log.Error( "Could not read pre-roll packet!  Try seeking again prior to reading more samples." );
					return;
				}
		...
		public long TotalSamples
		{
			get
			{
				...
				Log.Error( "ObjectDisposedException " + nameof( StreamDecoder ) );
				return 0;
			}
		}

...
namespace NVorbis
{
    ...
    class Floor0 : IFloor
    {
        ...
        public void Init(IPacket packet, int channels, int block0Size, int block1Size, ICodebook[] codebooks)
        {
            ...
            if (_order < 1 || _rate < 1 || _bark_map_size < 1 || _books.Length == 0) Log.Error("NVorbis::Init> Invalid data");
            ...
            for (int i = 0; i < _books.Length; i++)
            {
                ...
                if (num < 0 || num >= codebooks.Length) Log.Error( "NVorbis::Init> Invalid data" );
                var book = codebooks[num];

                if (book.MapType == 0 || book.Dimensions < 1) Log.Error( "NVorbis::Init> Invalid data" );

    ...
    class Mapping : IMapping
    {
        ...
        public void Init(IPacket packet, int channels, IFloor[] floors, IResidue[] residues, IMdct mdct)
        {
            ...
            for (var j = 0; j < couplingSteps; j++)
            {
                ...
                {
					Log.Error( "Invalid magnitude or angle in mapping header!" );
            ...
            {
				Log.Error( "Reserved bits not 0 in mapping header." );
            ...
            if (submapCount > 1)
            {
                for (var c = 0; c < channels; c++)
                {
                    ...
                    {
						Log.Error( "Invalid channel mux submap index in mapping header!" );
            ...
            for (var j = 0; j < submapCount; j++)
            {
                ...
                {
					Log.Error( "Invalid floor number in mapping header!" );
                ...
                {
                    Log.Error("Invalid residue number in mapping header!");
